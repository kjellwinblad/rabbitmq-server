...
-spec deliver_to_queues(rabbit_types:delivery(), map(),
                        [amqqueue:amqqueue()], state()) ->
    state().
...
redeliver(#pending{delivery = Msg} = Pend,
    ...
    % {array, [{table, MostRecentDeath}|_]} =
    %     rabbit_misc:table_lookup(Headers, <<"x-death">>),
    % {<<"routing-keys">>, array, Routes0} =
    %     lists:keyfind(<<"routing-keys">>, 1, MostRecentDeath),
        ...
        _ ->
            ...
            case RouteToQs of
                ...
                _ ->
                    ...
                    Options = #{correlation => OutSeq},
                    deliver_to_queues(Msg, Options, rabbit_amqqueue:lookup(RouteToQs), State)
            end
    end.
