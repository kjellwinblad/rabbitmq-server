...
%% AMQP legacy
-spec message(binary(), binary(), #content{}) -> mc:state().
message(ExchangeNameBin, RoutingKey, #content{properties = Props} = Content)
  when is_binary(ExchangeNameBin) andalso
       is_binary(RoutingKey) ->
    HeaderRoutes = rabbit_basic:header_routes(Props#'P_basic'.headers),
    mc:init(?MODULE,
            rabbit_basic:strip_bcc_header(Content),
            #{routing_keys => [RoutingKey | HeaderRoutes],
