...
%%
%% A <strong>deprecated feature</strong> is a name and several properties given
%% to a feature in RabbitMQ that will be removed in a future version. By
%% defining a deprecated feature, we can communicate to end users that what
%% they are using is going away and allow them to test how RabbitMQ will behave
...
%% '''
%%
%% == How to allow or deny a deprecated feature from the configuration ==
%%
%% The following configuration snippet permits one deprecated feature and
%% denies another one:
%%
%% ```
%% permit_deprecated_features.my_deprecated_feature_1 = true
%% permit_deprecated_features.my_deprecated_feature_2 = false
%% '''
...
%% lifecycle</li>
%% <li>`msg_when_permitted': what message to log and possibly display to the
%% user when the feature is being permitted and used. It is logged as a
%% warning or displayed to the user by the CLI or in the management UI for
%% instance.</li>
%% <li>`msg_when_denied': like `msg_when_denied', message used when an attempt
%% to use a denied deprecated feature is being made. It is logged as an error
%% or displayed to the user by the CLI or in the management UI for
%% instance.</li>
%% </ul>
%%
...
get_warning(FeatureProps, Permitted) when is_map(FeatureProps) ->
    ...
    Msgs = maps:get(msgs, FeatureProps),
    if
...
generate_warnings(FeatureName, FeatureProps) ->
    Msgs0 = maps:get(msgs, FeatureProps, #{}),
    Msgs1 = generate_warnings1(FeatureName, FeatureProps, Msgs0),
    FeatureProps#{msgs => Msgs1}.
    ...
    if
        Phase =:= permitted_by_default ->
            #{when_permitted =>
              rabbit_misc:format(
                ...
                "its use is currently permitted by default but it will be "
                "denied eventually.~n"
                "To keep this feature permitted when it becomes denied by "
                "default in a future version, set this in your "
                "configuration:~n"
                "  \"permit_deprecated_features.~ts = true\"~n"
                "To test RabbitMQ with this feature removed, set this in "
                "your configuration:~n"
                "  \"permit_deprecated_features.~ts = false\"",
                [FeatureName, FeatureName, FeatureName]),

              when_denied =>
              rabbit_misc:format(
                "Feature `~ts` is deprecated; "
                "its use is permitted by default but it is configured to "
                "be denied.~n"
        ...
        Phase =:= denied_by_default ->
            #{when_permitted =>
              rabbit_misc:format(
                "Feature `~ts` is deprecated; "
                "its use is denied by default but it is configured to "
                "be permitted.~n"
                "See the following setting in your configuration:~n"
                "  \"permit_deprecated_features.~ts = true\"~n"
                "It will be removed in a future version and it will not be "
                "possible to use this feature anymore, regardless of the "
                "configuration.",
                [FeatureName, FeatureName]),
              ...
              rabbit_misc:format(
                "Feature `~ts` is deprecated; "
                "its use is currently denied by default and it will be "
                "removed eventually.~n"
                "Once removed, it will not be possible to use this feature "
                "anymore, regardless of the configuration.~n"
                "To continue to use this feature before it is removed, set "
                "this in your configuration:~n"
                "  \"permit_deprecated_features.~ts = true\"",
                [FeatureName, FeatureName])};
